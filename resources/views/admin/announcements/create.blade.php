        ...
        <div class="w-5/6 bg-gray-900">
            <div class="p-3 w-full flex flex-col justify-center">
                ...
                <div class="flex flex-row justify-center h-full">
                    <form action="{{ route('announcements.store') }}" method="post" class="max-w-md w-full flex flex-col">
                        @csrf
                        @method('POST')
                        ...
                        <div class="mb-4 flex justify-between">
                            <div>
                                ...
                                <input type="datetime-local" name="start_date" id="start_date"
                                    class="form-input w-full rounded-md dark:bg-gray-800 dark:text-gray-300" value="{{ old('start_date') }}">
                                @error('start_date')
                                    <span class="text-red-500">{{ $message }}</span>
                                @enderror
                            </div>
                            ...
                            <div>
                                ...
                                <input type="datetime-local" name="end_date" id="end_date"
                                    class="form-input w-full rounded-md dark:bg-gray-800 dark:text-gray-300" value="{{ old("end_date") }}">
                                @error('end_date')
                                    <span class="text-red-500">{{ $message }}</span>
                                @enderror
                            </div>
                        ...
                        <div class="mb-4">
                            ...
                            <textarea name="description" id="myeditorinstance"
                                class="form-textarea w-full rounded-md dark:bg-gray-800 dark:text-gray-300">{{ old('description') }}</textarea>
                            @error('description')
                                <span class="text-red-500">{{ $message }}</span>
                            @enderror
                        </div>
                        ...
                        <div class="mb-4">
                            ...
                            <select name="parteners[]" class="form-select w-full rounded-md dark:bg-gray-800 dark:text-gray-300" id="multiple-select-clear-field"
